...
class OverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Calculate track area (right upper corner, max 50% of width and height)
    final maxTrackWidth = size.width * 0.5;
    final maxTrackHeight = size.height * 0.5;
    final padding = 20.0;
    ...
    final trackOffsetX = size.width - areaWidth - padding;
    final trackOffsetY = padding;
  ...
  void _drawPosition(Canvas canvas, double offsetX, double offsetY, double areaHeight, GpsBounds bounds, double scale) {
    ...
    canvas.drawCircle(Offset(x, y), positionRadius, outerPaint);
    
    final innerPaint = Paint();
    innerPaint.color = Colors.white;
    innerPaint.style = PaintingStyle.fill;
    canvas.drawCircle(Offset(x, y), positionRadius * 0.5, innerPaint);
    
    final borderPaint = Paint();
    borderPaint.color = positionColor;
    borderPaint.strokeWidth = 2;
    borderPaint.style = PaintingStyle.stroke;
    canvas.drawCircle(Offset(x, y), positionRadius, borderPaint);
  }
  ...
  void _drawPaceInfo(Canvas canvas, Size size) {
    ...
    // Box width: almost full width with margins (20px on each side)
    final boxWidth = size.width - 40;
    
    // Smaller text size (just slightly larger than base)
    final fontSize = textSize * 0.9;
    
    // Calculate text dimensions
    final lineHeight = fontSize * 1.6;
    final totalHeight = lineHeight * 4 + 25; // 4 lines + padding
    
    // Background - positioned at bottom with margins
    final bgPaint = Paint();
    bgPaint.color = Colors.black.withOpacity(0.6);
    bgPaint.style = PaintingStyle.fill;
    
    final bgRect = Rect.fromLTWH(20, size.height - totalHeight - 20, boxWidth, totalHeight);
    canvas.drawRRect(
    ...
    final startY = size.height - totalHeight - 5;
    ...
    textPainter.paint(canvas, Offset(30, startY));
  }

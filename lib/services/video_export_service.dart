...
class VideoExportService {
  Future<String?> exportVideoWithOverlay({
    ...
    if (kIsWeb) {
      print('Video export not supported on web');
      return null;
    }

    if (Platform.isAndroid || Platform.isIOS) {
      print('Video export requires ffmpeg-kit AAR on Android/iOS');
      print('Download ffmpeg-kit-min-6.0-2.aar and place in android/app/libs/');
      print('See: https://github.com/nicknoun/ffmpeg-kit/releases');
      return null;
    }

    if (!Platform.isWindows && !Platform.isLinux && !Platform.isMacOS) {
      print('Video export only supported on desktop platforms');
      return null;
    }
  ...
  Future<String?> _exportDesktop({
    ...
    try {
      ...
      if (extractResult.exitCode != 0) {
        print('Frame extraction failed: ${extractResult.stderr}');
      ...
      if (encodeResult.exitCode == 0) {
        ...
        print('Video encoding failed: ${encodeResult.stderr}');
        return null;
      }
    } catch (e) {
